...
static inline bool clk_master_ready_a7(struct clk_master *master)
{
				   ...
				   unsigned int status)
{
	...
	while ((cparent != master->parent) && !clk_master_ready_a7(master))
		;
}

static TEE_Result clk_sama7g5_master_set_rate(struct clk *hw, unsigned long rate,
				       unsigned long parent_rate)
{
	struct clk_master *master = hw->priv;
	unsigned long div;
