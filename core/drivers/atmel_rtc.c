...
#define RTC_SET_VAL(reg, val )	(((val) & RTC_## reg ##_MASK) << \
				 RTC_## reg ## _SHIFT) 
...
static void atmel_decode_date(unsigned time_reg, unsigned int cal_reg,
...
void atmel_rtc_get_tamper_timestamp(void)
{
	...
	source = atmel_rtc_read(RTC_TSSR0);

	for (i = 0; i < RTC_TSSR_DET_COUNT; i++) {
		if (source & BIT(RTC_TSSR_DET_OFFSET + i))
			EMSG("Tampering source %d\n", i);
	}

	if (source & RTC_TSSR_JTAG)
		EMSG("JTAG tampering attempt\n");

	if (source & RTC_TSSR_TST_PIN)
			      ...
			      struct optee_rtc_time *tm)
{
	...
	atmel_rtc_write(RTC_CALR,
			  RTC_SET_VAL(CAL_CENT,
				     bcd_encode((tm->tm_year + 1900) / 100))
...
static TEE_Result atmel_rtc_get_offset(struct rtc *rtc __unused, long *offset)
{
	...
	*offset = DIV_ROUND_NEAREST(ATMEL_RTC_CORR_DIVIDEND, val);
...
static TEE_Result atmel_rtc_set_offset(struct rtc *rtc  __unused, long offset)
{
	 ...
	 */
	if (offset < 29208) {
		corr = DIV_ROUND_NEAREST(ATMEL_RTC_CORR_DIVIDEND,
					 offset * ATMEL_RTC_CORR_LOW_RATIO);
	} else {
		corr = DIV_ROUND_NEAREST(ATMEL_RTC_CORR_DIVIDEND, offset);
		mr |= RTC_MR_HIGHPPM;
	}
...
static struct rtc atmel_rtc = {
	...
	.range_min = TIME_INIT(1900, 1, 1, 0, 0, 0),
	.range_max = TIME_INIT(2099, 12, 31, 23, 59, 59),
};
				      ...
				      const void *compat_data __unused)
{
	...
	/* Enable 24 hours Gregorian mode */
	io_clrbits32(rtc_base + RTC_MR, RTC_MR_PERSIAN | RTC_MR_UTC |
		     RTC_MR_HR_MODE);

	return rtc_register(&atmel_rtc);
}
